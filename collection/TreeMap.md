...
HashSet内部使用的是HashMap，而TreeSet内部使用的是TreeMap，这两个Map，当第一次插入元素时，返回的时null，而修改元素时返回的是key对应的value值，假如这两个Set使用null作为value值，那么第一次调用add方法后返回的是true，而当再修改的时候，由于value值存放的是null，那么add方法返回的也是true，与它要表现的与行为是不一致，即插入重复的元素应该返回失败。

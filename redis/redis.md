...
- 正常情况下，当哈希表中的元素个数大于等于数组大小（哈希表由数组+链表组成，当总元素大于等于数组长度时就会扩容，也称为负载因子大于等于1）时就会进行扩容，新的容量为第一个大于等于总元素大小x2的值 （2 的 n 次方幂），比如当前总元素为11个，那么新的容量必须要大于11x2=22 并且时2的n 次方幂，那就是32
